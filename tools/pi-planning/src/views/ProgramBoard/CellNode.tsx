...
function CellNodeInner({ data }: NodeProps) {
  const { used, available, isActive, isIP, dropHalf, dropBlocked } = data as {
    teamId: string;
    iterationId: string;
    used: number;
    available: number;
  ...
  return (
    <div className={`rf-cell ${isActive ? 'rf-cell--active' : ''} ${isIP ? 'rf-cell--ip' : ''} ${dropBlocked ? 'rf-cell--blocked' : ''}`}>
      {/* W1/W2 halves */}
