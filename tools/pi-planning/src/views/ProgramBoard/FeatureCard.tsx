...
const DEP_COLORS: Record<string, { border: string; bg: string; fg: string }> = {
  ...
  milestone:   { border: '#f59e0b', bg: 'rgba(245,158,11,0.08)', fg: '#d97706' },
};
  ...
  return (
    ...
    >
      ...
      <div className="rf-card__head">
        ...
        {!isMilestone && item.js != null && <span className="rf-card__js">JS {item.js}</span>}
        {!isMilestone && item.wsjf != null && <span className="rf-card__wsjf">W {item.wsjf.toFixed(1)}</span>}
      </div>
      ...
      <div className="rf-card__foot">
        ...
        <span className="rf-card__status">{isMilestone ? item.type : item.status}</span>
      </div>
    </div>
  );
}
