...
Usage:
    ...
    python scripts/edpa_sync.py status        # Show sync status
...
def load_sync_config(root):
    """Load sync configuration from .edpa/config.yaml."""
    config_path = root / ".edpa" / "config.yaml"
...
def compute_diff(local_items, remote_items):
    """
    Compare local (backlog.yaml) and remote (GitHub Project) items.
    ...
    for item_id in sorted(all_ids):
        ...
        if local and not remote:
            changes.append({
                "id": item_id,
                "action": "local_only",
                "detail": f"Exists in backlog.yaml but not in GitHub Project",
                "local": local,
            })
            continue
        ...
        if remote and not local:
            changes.append({
                "id": item_id,
                "action": "remote_only",
                "detail": f"Exists in GitHub Project but not in backlog.yaml",
                "remote": remote,
            })
            continue
...
def cmd_diff(root, sync_config, args):
    ...
    if remote_only:
        print(bold(color("  Remote only (not in backlog.yaml):", C.DIFF_DEL)))
...
def cmd_conflicts(root, _sync_config, args):
    ...
    print(color("  Resolution: manually edit .edpa/backlog.yaml and run `push`.", C.MUTED))
    print()
