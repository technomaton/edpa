...
def _commit_evidence(repo_root: Path, paths: list[Path],
    ...
    """Stage + commit the post-commit-hook evidence changes."""
    ...
    return _git_commit_paths(repo_root, paths, msg)


def _transition_signals(transitions: list[dict], people: list[dict],
                        default_pid: str | None = None) -> dict[str, list[dict]]:
    """Resolve + convert transition dicts into ``{item_id: [signal]}`` for
    evidence[]. Author email → person id via people.yaml; falls back to
    ``default_pid`` (the hook's already-resolved commit author)."""
...
def _load_yaml_edit_weights(edpa_root: Path):
    """Project yaml_edit weight overrides from cw_heuristics.yaml (or None)."""
    h = edpa_root / "config" / "cw_heuristics.yaml"
    if not h.exists():
        return None
    import yaml
...
def _yaml_edit_to_evidence(raw: dict, people: list,
    ...
    evidence[]-shaped signals: person-keyed, `at` timestamp, carrying the
    structural `delta` + raw_weight/discount for audit/analytics."""
    grouped: dict[str, list[dict]] = {}
    for iid, sigs in raw.items():
        for s in sigs:
            ...
            grouped.setdefault(iid, []).append({
                ...
                "raw_weight": s.get("raw_weight"),
                "discount": s.get("discount"),
                "ref": s.get("ref"),
                "at": s.get("detected_at"),
                "tags": s.get("tags", []),
                "delta": s.get("delta", {}),
            })
    return grouped
...
def cmd_materialize(edpa_root: Path, iteration_id: str) -> int:
    ...
    Catches commits the post-commit hook never saw — runs with
    EDPA_NO_LOCAL_EVIDENCE=1, history predating the hook, or commits from
    another machine. Dedup is by signal ``ref`` (commit hash + from→to),
    so re-running is a no-op. This is the explicit catch-up path and does
    NOT honour EDPA_NO_LOCAL_EVIDENCE."""
    edpa_root = Path(edpa_root)
    repo_root = edpa_root.parent  # cwd-independent (MCP calls from any cwd)
    from transitions import detect_transitions, parse_iteration_dates
    iter_file = edpa_root / "iterations" / f"{iteration_id}.yaml"
    if not iter_file.is_file():
        print(f"materialize: iteration {iteration_id!r} not found",
              file=sys.stderr)
        return 1
    since, until = parse_iteration_dates(iter_file)
    people = _load_people(edpa_root)
    trans = detect_transitions(edpa_root, since, until)
    grouped = _transition_signals(trans, people)

    # yaml_edit over the same window (variant-2 discount applied in collector).
    from yaml_edit_signals import collect_yaml_edit_signals
    ye = collect_yaml_edit_signals(edpa_root, iteration_id,
                                   _load_yaml_edit_weights(edpa_root))
    for iid, sigs in _yaml_edit_to_evidence(ye, people).items():
    ...
    if touched:
        msg = (f"{_SELF_COMMIT_PREFIX} materialize {iteration_id} "
               f"({len(touched)} item(s))\n\n"
               f"Auto-generated by local_evidence.py --materialize "
               f"(state_transition + yaml_edit signals).")
        _git_commit_paths(repo_root, touched, msg)
    print(f"materialize: {len(touched)} item(s) updated for {iteration_id} "
          f"({len(trans)} transitions, "
          f"{sum(len(v) for v in ye.values())} yaml_edit scanned)")
    return 0
...
def main(argv=None) -> int:
    # Lenient manual parse (NOT argparse): the post-commit hook calls main()
    # in-process and unit tests call main() while pytest's own argv is still
    # on sys.argv — argparse would SystemExit on those. We only need to spot
    ...
    # Phase 1: materialize state_transition signals from THIS commit
    # (analytics, weight 0 — does not change cw). Same dedup-by-ref path.
    try:
        from transitions import detect_transitions_for_commit
        trans = detect_transitions_for_commit(edpa_root, commit["sha"])
        for iid, sigs in _transition_signals(
                trans, people, default_pid=person_id).items():
            grouped.setdefault(iid, []).extend(sigs)
    except Exception as exc:  # noqa: BLE001
        print(f"local_evidence: transition detection skipped: {exc}",
              file=sys.stderr)

    # Phase 2: materialize yaml_edit signals from THIS commit (structural
    # diff over backlog YAMLs; variant-2 discount applied in the collector).
    try:
        from yaml_edit_signals import collect_yaml_edit_signals_for_commit
        ye = collect_yaml_edit_signals_for_commit(
            edpa_root, commit["sha"], _load_yaml_edit_weights(edpa_root))
        for iid, sigs in _yaml_edit_to_evidence(
                ye, people, default_pid=person_id).items():
            grouped.setdefault(iid, []).extend(sigs)
    except Exception as exc:  # noqa: BLE001
