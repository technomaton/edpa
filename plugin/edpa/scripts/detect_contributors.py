...
# D-66: subject prefix of the machine-bookkeeping commit created by
# ``--all-items --commit``. check_ticket_attached whitelists it (no ticket
# to demand) and local_evidence skips it (a contributors[] rewrite touches
# every item with evidence[] — crediting its author would spray
# commit_author weight across the whole backlog at close time).
CONTRIB_COMMIT_PREFIX = "chore(contributors):"
_DEFAULT_COMMIT_CONTEXT = "all-items scan"
...
def _commit_contributor_refresh(edpa_root: Path, paths: list[Path],
                                context: str) -> bool:
    """Commit ONLY the contributor-rewritten files (machine bookkeeping).

    D-66: ``--all-items`` used to leave its rewrites uncommitted; the close
    commit then swept every touched backlog file, and the post-commit
    evidence hook credited the closer with commit_author on all of them.
    The bookkeeping commit is:

      * pathspec-limited (``git commit … -- <paths>``) — unrelated staged
        work stays staged, same idiom as local_evidence._git_commit_paths;
      * run with ``EDPA_NO_LOCAL_EVIDENCE=1`` in the subprocess env so the
        post-commit evidence hook stays silent — machine bookkeeping must
        not create attribution (check_ticket_attached / local_evidence /
        yaml_edit_signals additionally skip the subject prefix, covering
        manual commits under the same subject);
      * ``--no-verify`` — bypasses arbitrary user pre-commit hooks the same
        way the chore(evidence): auto-commit does.
    """
    out = subprocess.run(
        ["git", "rev-parse", "--show-toplevel"],
        cwd=str(Path(edpa_root).resolve().parent),
        capture_output=True, text=True, encoding="utf-8",
    )
    top = (out.stdout or "").strip()
    if out.returncode != 0 or not top:
        print("WARN: --commit requested outside a git repo — refreshed "
              "files left uncommitted.", file=sys.stderr)
        return False
    ...
    for p in paths:
        rp = Path(p).resolve()
        try:
            rels.append(str(rp.relative_to(repo_root)))
        except ValueError:
            rels.append(str(rp))  # still valid as an absolute pathspec
    env = {**os.environ, "EDPA_NO_LOCAL_EVIDENCE": "1"}
    msg = (f"{CONTRIB_COMMIT_PREFIX} refresh after {context}\n\n"
           f"Auto-generated by detect_contributors.py --all-items --commit.\n"
           f"Machine bookkeeping (contributors[] recomputed from evidence[]);\n"
           f"evidence hook suppressed via EDPA_NO_LOCAL_EVIDENCE=1.")
    add = subprocess.run(
        ["git", "add", "--", *rels], cwd=str(repo_root), env=env,
        capture_output=True, text=True, encoding="utf-8",
    )
    if add.returncode != 0:
        return False
...
def cmd_all_items(edpa_root: Path, dry_run: bool = False, *,
                  commit: bool = False,
                  context: str = _DEFAULT_COMMIT_CONTEXT) -> int:
    ...
    ``commit=True`` (D-66): commit ONLY the rewritten files as
    ``chore(contributors): refresh after <context>`` with the evidence
    hook suppressed — see _commit_contributor_refresh. Without it the
    rewrites stay in the working tree, and sweeping them into a later
    close commit makes the post-commit hook credit the closer with
    commit_author on every touched item.
    """
    weights = load_signal_weights(edpa_root)
    people_map = load_people_map(edpa_root)
        ...
        for f in sorted(dir_path.glob("*.md")):
            ...
            try:
                ...
                if changed and not dry_run:
                    # Same lookup process_item used for the write, so the
                    # committed path is the one actually rewritten (a stale
                    # duplicate stem in another type dir resolves canonically).
                    p = find_backlog_file(edpa_root, item_id)
                    if p is not None:
    ...
    if commit and dry_run:
        print("--commit: ignored under --dry-run (nothing was written)")
    elif commit:
        if not changed_paths:
            print("--commit: contributors[] already current — nothing to commit")
        elif _commit_contributor_refresh(edpa_root, changed_paths, context):
            print(f"--commit: {len(changed_paths)} file(s) committed as "
                  f"'{CONTRIB_COMMIT_PREFIX} refresh after {context}'")
        else:
            print(f"ERROR: --commit failed — {len(changed_paths)} rewritten "
                  f"file(s) left uncommitted. Commit them yourself under a "
                  f"'{CONTRIB_COMMIT_PREFIX} …' subject; never sweep them "
                  f"into a close commit (the post-commit hook would credit "
                  f"the closer on every touched item).", file=sys.stderr)
            return 1
    return 0
...
def main():
    ...
    ap.add_argument("--commit", action="store_true",
                    help="With --all-items (D-66): commit ONLY the rewritten "
                         "files as 'chore(contributors): refresh after "
                         "<context>' — machine bookkeeping with the evidence "
                         "hook suppressed (EDPA_NO_LOCAL_EVIDENCE=1)")
    ap.add_argument("--context", default=None,
                    help="With --all-items --commit: context for the "
                         "auto-commit subject, e.g. 'PI-2026-1.3 close' "
                         f"(default: '{_DEFAULT_COMMIT_CONTEXT}')")
    args = ap.parse_args()

    if args.commit and not args.all_items:
        ap.error("--commit requires --all-items")
    ...
    if args.all_items:
        return cmd_all_items(edpa_root, dry_run=args.dry_run,
                             commit=args.commit,
                             context=args.context or _DEFAULT_COMMIT_CONTEXT)
