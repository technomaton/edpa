...
| `edpa_iteration_close` | Mark an iteration closed (audit state) + stamp `delivery.delivered_sp`/`velocity` from Done items; never writes `planning.planned_sp` (stamped at planning time, predictability n/a when missing) | `/edpa:close-iteration` |
