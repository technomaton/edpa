...
`compute_cw()` v engine čte `role_overrides` z `.edpa/config/heuristics.yaml` a aplikuje je podle `person.role`. CW hodnoty se změní pro non-Dev role:
...
2. **Volitelně přidat `contract` pole** do `.edpa/config/people.yaml` pro multi-contract
...
- Všechny v1.x `.edpa/config/people.yaml` soubory fungují beze změny
